...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        String numProfile= StdIn.readLine();
        int numProfiles = Integer.parseInt(numProfile);
        database= new Profile[numProfiles];
        for(int i=0; i<numProfiles; i++){
            String name=StdIn.readLine();
            String s1=StdIn.readLine();
            String s2=StdIn.readLine();
            Profile profile = new Profile(name,null, null, s1, s2);
            database[i]= profile;
        }
    }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        int numSTRs=Integer.parseInt(s);
   ...
   String sequence2=StdIn.readLine();
        ...
        return (new Profile("Unknown", null, null, sequence1, sequence2)); // update the return value
    }
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        String sub=sequence.substring(i, i+STR.length());
        if(sub.equals(STR)){
            current++;
        ...
        return new STR(STR, longest); // update the return value
    }
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        // Loop through the database array and get each profile
        for (int i = 0; i < database.length; i++) {
          Profile profile = database[i];
          STR[] profileS1_STRs = profile.getS1_STRs();
          boolean match = identicalSTRs(profileS1_STRs, unknownProfileS1_STRs);
          if (match) {
            matches.add(profile);
          }
        }
         ...
         for (int p1 = 0; p1 < possibleParent1.size(); p1++){
            for (int p2 = 0; p2 < possibleParent2.size(); p2++){
                if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2))){
                    if ( punnetSquare(possibleParent2.get(p2).getS2_STRs(), S1_STRs, possibleParent1.get(p1).getS1_STRs(), S2_STRs)){
